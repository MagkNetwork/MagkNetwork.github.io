...
function close_login() {
    ...
    history.pushState("", document.title, window.location.pathname + window.location.search); // remove link with '#'
}
...
async function popup_metamask() {
    ...
    if(currentAccount.length === 0){
        alert('Please agree the metamask connection!');
    } else if(currentAccount.length === 1) {
        document.getElementsByClassName('login_button')[0].innerHTML = currentAccount[0].slice(0, 4) + '...' + currentAccount[0].slice(-4);
        document.getElementsByClassName('menu-login-btn')[0].style.pointerEvents = "none";
        toggle_login_arrow();
        CURRENT_LOGIN_METHOD = 3;
        CURRENT_USER_ACCOUNT = currentAccount[0];
    }
